...
Helpers for analysing
[OpenMalaria](https://github.com/OpenMalaria-Org/openmalaria) output from
Python.
...
- `by_age_group(survey, measure)`: values of one measure as an (age group x
  survey) array. `measure` is a `MEASURE_CODES` name or its integer code.
...
- `pfpr(survey, upperbounds, lo=2, hi=10, survey_index=-1)`: parasite prevalence
  over the monitoring age groups lying entirely within `[lo, hi]`.
